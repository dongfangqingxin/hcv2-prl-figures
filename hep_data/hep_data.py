"""
script for creating HepData submission for the paper on charm hadron v2 in PbPb at 5.36 TeV
corresponding sandbox: https://www.hepdata.net/record/sandbox/1773329189
install hepdata_lib: python -m pip install hepdata_lib
need to use ./__init__.py to instead orginal __init__.py of hepdata_lib
"""
...
def fig1():
    ...
    ###############################
    ### D0 30-50% PbPb 5.36 TeV ###
    ###############################

    tableFig1_Dzero = Table("Figure 1 Dzero 30-50% PbPb 5.36 TeV")
    description_Dzero = "$p_\mathrm{T}$-differential prompt $\mathrm{D}^{0}$ $v_2${SP} at midrapidity ($|y|<0.8$) in the 30-50% centrality class in Pb-Pb collisions at $\sqrt{s_{NN}}$ = 5.36 TeV"
    tableFig1_Dzero.description = description_Dzero
    tableFig1_Dzero.location = "Data from Figure 1 $\mathrm{D}^{0}$ (azure solid circles)"
    tableFig1_Dzero.keywords["observables"] = ["$v_2(\mathrm{D}^{0})$ in $|y|<0.8$"]
    tableFig1_Dzero.keywords["cmenergies"] = ["5360.0"]
    tableFig1_Dzero.keywords["reactions"] = ['PbPb --> $\mathrm{D}^{0}$ + X']
    Dzero = readerFig1_Dzero.read_graph(statName)
    Dzero_syst = readerFig1_Dzero.read_graph(systName)

    # x axis
    xDzero = Variable("PT", is_independent=True, is_binned=False, has_weighted_bins=True, units="GeV/c")
    xDzero.values = [(num, num+err_low, num+err_high) for (num, (err_low, err_high)) in zip(Dzero['x'], Dzero['dx'])]
    
    # y axis
    yDzero = Variable("$v_2\{SP\}^{\mathrm{D}^{0}}|_{|y|<0.8}$", is_independent=False, is_binned=False, units="")
    yDzero.add_qualifier('CENTRALITY', '30.0-50.0 pct')
    ...
    yerrDzero = Uncertainty("stat", is_symmetric=False)
    yerrDzero.values = [(round(num[0], round_digits), round(num[1], round_digits)) for num in Dzero["dy"]]
    ...
    ###############################
    ### D+ 30-50% PbPb 5.36 TeV ###
    ###############################

    tableFig1_Dplus = Table("Figure 1 Dplus 30-50% PbPb 5.36 TeV")
    description_Dplus = "$p_\mathrm{T}$-differential prompt $\mathrm{D}^{+}$ $v_2${SP} at midrapidity ($|y|<0.8$) in the 30-50% centrality class in Pb-Pb collisions at $\sqrt{s_{NN}}$ = 5.36 TeV"
    tableFig1_Dplus.description = description_Dplus
    tableFig1_Dplus.location = "Data from Figure 1 $\mathrm{D}^{+}$ (green solid diamonds)"
    tableFig1_Dplus.keywords["observables"] = ["$v_2(\mathrm{D}^{+})$ in $|y|<0.8$"]
    tableFig1_Dplus.keywords["cmenergies"] = ["5360.0"]
    tableFig1_Dplus.keywords["reactions"] = ['PbPb -->$\mathrm{D}^{+}$ + X']

    Dplus = readerFig1_Dplus.read_graph(statName)
    Dplus_syst = readerFig1_Dplus.read_graph(systName)

    # x axis
    xDplus = Variable("PT", is_independent=True, is_binned=False, has_weighted_bins=True, units="GeV/c")
    xDplus.values = [(num, num+err_low, num+err_high) for (num, (err_low, err_high)) in zip(Dplus['x'], Dplus['dx']) ]
    
    # y axis
    yDplus = Variable("$v_2\{SP\}^{\mathrm{D}^{+}}|_{|y|<0.8}$", is_independent=False, is_binned=False, units="")
    yDplus.add_qualifier('CENTRALITY', '30.0-50.0 pct')
    ...
    # statistical uncertainty
    yerrDplus = Uncertainty("stat", is_symmetric=False)
    yerrDplus.values = [(round(num[0], round_digits), round(num[1], round_digits)) for num in Dplus["dy"]]
    ...
    ################################
    ### Ds+ 30-50% PbPb 5.36 TeV ###
    ################################

    tableFig1_Ds = Table("Figure 1 Ds 30-50% PbPb 5.36 TeV")
    description_Ds = "$p_\mathrm{T}$-differential prompt $\mathrm{D}^{+}_\mathrm{s}$ $v_2${SP} at midrapidity ($|y|<0.8$) in the 30-50% centrality class in Pb-Pb collisions at $\sqrt{s_{NN}}$ = 5.36 TeV"
    tableFig1_Ds.description = description_Ds
    tableFig1_Ds.location = "Data from Figure 1 $\mathrm{D}^{+}_\mathrm{s}$ (violet solid crosses)"
    tableFig1_Ds.keywords["observables"] = ["$v_2(\mathrm{D}_{\mathrm{s}}^{+})$ in $|y|<0.8$"]
    tableFig1_Ds.keywords["cmenergies"] = ["5360.0"]
    tableFig1_Ds.keywords["reactions"] = ['PbPb --> $\mathrm{D}^{+}_\mathrm{s}$ + X']

    Ds = readerFig1_Ds.read_graph(statName)
    Ds_syst = readerFig1_Ds.read_graph(systName)

    # x axis
    xDs = Variable("PT", is_independent=True, is_binned=False, has_weighted_bins=True, units="GeV/c")
    xDs.values = [(num, num+err_low, num+err_high) for (num, (err_low, err_high)) in zip(Ds['x'], Ds['dx']) ]

    # y axis
    yDs = Variable("$v_2\{SP\}^{\mathrm{D}^{+}_\mathrm{s}}|_{|y|<0.8}$", is_independent=False, is_binned=False, units="")
    yDs.add_qualifier('CENTRALITY', '30.0-50.0 pct')
    ...
    # statistical uncertainty
    yerrDs = Uncertainty("stat", is_symmetric=False)
    yerrDs.values = [(round(num[0], round_digits), round(num[1], round_digits)) for num in Ds["dy"]]
    ...
    ################################
    ### Lc 30-50% PbPb 5.36 TeV ####
    ################################
    
    tableFig1_Lc = Table("Figure 1 LambdaC 30-50% PbPb 5.36 TeV")
    description_Lc = "$p_\mathrm{T}$-differential prompt $\Lambda^+_\mathrm{c}$ $v_2${SP} at midrapidity ($|y|<0.8$) in the 30-50% centrality class in Pb-Pb collisions at $\sqrt{s_{NN}}$ = 5.36 TeV"
    tableFig1_Lc.description = description_Lc
    tableFig1_Lc.location = "Data from Figure 1 $\Lambda^+_\mathrm{c}$ (red solid squares)"
    tableFig1_Lc.keywords["observables"] = ["$v_2(\Lambda^+_\mathrm{c})$ in $|y|<0.8$"]
    tableFig1_Lc.keywords["cmenergies"] = ["5360.0"]
    tableFig1_Lc.keywords["reactions"] = ['PbPb --> $\Lambda^+_\mathrm{c}$ + X']

    Lc = readerFig1_Lc.read_graph(statName)
    Lc_syst = readerFig1_Lc.read_graph(systName)

    # x axis
    xLc = Variable("PT", is_independent=True, is_binned=False, has_weighted_bins=True, units="GeV/c")
    xLc.values = [(num, num+err_low, num+err_high) for (num, (err_low, err_high)) in zip(Lc['x'], Lc['dx']) ]

    # y axis
    yLc = Variable("$v_2\{SP\}^{\Lambda^+_\mathrm{c}}|_{|y|<0.8}$", is_independent=False, is_binned=False, units="")
    yLc.add_qualifier('CENTRALITY', '30.0-50.0 pct')
    yLc.add_qualifier('REACTION', 'PbPb --> $\Lambda^+_\mathrm{c}$ + X')
    yLc.add_qualifier('SQRT(S)', '5360 GEV')
    yLc.add_qualifier('YRAP', '-0.8 TO 0.8')
    yLc.values = [round(num, round_digits) for num in Lc["y"]]

    # statistical uncertainty
    yerrLc = Uncertainty("stat", is_symmetric=False)
    yerrLc.values = [(round(num[0], round_digits), round(num[1], round_digits)) for num in Lc["dy"]]
...
def sub():
    ...
    submission.comment = "The ALICE collaboration reports the azimuthal-anisotropy coefficient $v_2$ of prompt $\mathrm{D}^0$, $\mathrm{D}^+$, $\mathrm{D}^+_\mathrm{s}$ mesons and the first measurement of $v_2$ of prompt $\Lambda^+_\mathrm{c}$ baryons in semicentral Pb-Pb collisions at a center-of-mass energy per nucleon pair of $\sqrt{s_{NN}}=5.36$ TeV. The D mesons and $\Lambda^+_\mathrm{c}$baryons are reconstructed in their hadronic decays at midrapidity ($|y|<0.8$) in the transverse-momentum interval 0.5 $< p_\mathrm{T} <$ 24 GeV/c. Similar $v_2$ values are measured for $\mathrm{D}^0$ and $\mathrm{D}^+$, while a hint of a difference ($2.6\sigma$) emerges between $\mathrm{D}^0$ and $\mathrm{D}^+_\mathrm{s}$ mesons in the $1 <p_\mathrm{T} < 5~GeV/c$ interval. A larger $v_2$ for $\Lambda^+_\mathrm{c}$ baryons with respect to $\mathrm{D}^0$ mesons is observed with $3.7\sigma$ significance for $4 < p_\mathrm{T} < 12~GeV/c$, providing evidence for the partonic origin of charm-hadron $v_2$ and hadron formation via quark coalescence. This interpretation is further supported by comparisons with theoretical calculations of charm-quark transport in a hydrodynamically expanding medium. "
    ### figure 1
